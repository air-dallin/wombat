...
@section('title')
  ...
  @include('frontend.profile.sections.document_create_menu',['currentMenuType'=>'factura'])
        ...
        <div class="card">
          <div class="card-body">
            <form method="POST" id="create_product"
                  @isset($product)
                    action="{{localeRoute('frontend.profile.modules.product.update',$product)}}"
                  @else
                    action="{{localeRoute('frontend.profile.modules.product.store')}}"
                @endisset
            >
              @csrf
              ...
              <div class="p-5 mb-5 rounded-lg bg-white dark:bg-darkblack-600">
                <div class="grid grid-cols-1 gap-6 2xl:grid-cols-4">
                      ...
                      @isset($plans)
                        @foreach($plans as $plan)
                          <option value="{{$plan->id}}"
                              @if((isset($product) && $product->plan_id==$plan->id) || (!empty($selected_plan) && $selected_plan->plan_id==$plan->id)) selected @endif >{{$plan->code . ' - ' . $plan->title_ru }}
                          </option>
                        @endforeach
                      @endisset
                  ...
                  <div class="flex flex-col gap-2">
                    ...
                    <input type="text" name="number" id="number" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white @error('number') is-invalid @enderror" placeholder="" value="{{old('number',isset($product) ? $product->number : $new_number )}}" required>
                    @error('number')
                    <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                  ...
                  <div class="flex flex-col gap-2">
                    ...
                    <input type="date" name="date" id="date" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white @error('date') is-invalid @enderror" placeholder="" value="{{old('date',isset($product) ? date('Y-m-d',strtotime($product->date)) :'')}}" required>
                    @error('date')
                    <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                ...
                <div class="p-5 mb-5 rounded-lg bg-white dark:bg-darkblack-600">
                    <div class="grid grid-cols-1 gap-6 2xl:grid-cols-2">
                            ...
                            <div class="flex flex-col gap-2">
                                ...
                                <select class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white @error('ttn_id') is-invalid @enderror" name="ttn_id" id="ttn_id">
                            ...
                            <div class="flex flex-col gap-2">
                                ...
                                <select class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white @error('old_factura_id') is-invalid @enderror" name="old_factura_id">
                                    ...
                                    @foreach($factures as $factura)
                                        <option value="{{$factura->id}}" @if(isset($product) && $product->old_facture_id==$factura->id) selected @endif >{{ $factura->number . ' (' . date('Y-m-d',strtotime($factura->date) ) }})</option>
                                    @endforeach
                                    @endisset
                                </select>
                                @error('contract_id')
                  ...
                  <div>
                    ...
                    <div class="grid grid-cols-1 gap-6 2xl:grid-cols-2">
                      <div class="flex flex-col mt-3 mb-3 gap-2">
                        ...
                        <input type="text" name="company_inn"
                               ...
                               placeholder=""
                               value="{{old('company_inn',isset($product) ? $product->company_inn :'')}}">
                        @error('company_inn')
                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                      ...
                      <div class="flex flex-col mt-3 mb-3 gap-2">
                        ...
                        <select class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white" name="company_id" id="company_id" required>
                          ...
                          @foreach($companies as $_company)
                            <option value="{{$_company->id}}" @if( (isset($product) && $product->company_id==$_company->id) || $_company->id==$current_company) selected @endif >{{$_company->name}}</option>
                          @endforeach
                        </select>
                        @error('company_id')
                        <span class="invalid-feedback" role="alert">
                    ...
                    <div class="grid grid-cols-1 gap-6 2xl:grid-cols-2">
                      <div class="flex flex-col mb-3  gap-2">
                        ...
                        <select class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white @error('company_invoice_id') is-invalid @enderror"
                          ...
                          @isset($product->company->invoices)
                            @foreach($product->company->invoices as $invoice)
                              <option value="{{$invoice->id}}"
                                      @if(isset($product) && $product->company_account==$invoice->bank_invoice) selected @endif >{{$invoice->bank_invoice}}</option>
                                      {{--@if(isset($product) && $product->company_invoice_id==$invoice->id) selected @endif >{{$invoice->bank_invoice}}</option>--}}
                            @endforeach
                          @endisset
                        </select>
                      ...
                      <div class="flex flex-col mb-3  gap-2">
                        ...
                        <input type="text" name="company_mfo" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white" placeholder=""
                               value="{{old('company_mfo',isset($product) ? $product->company_mfo :'')}}" required>
                        @error('company_mfo')
                    ...
                    <div class="flex flex-col mb-3  gap-2">
                      ...
                      <input type="text" name="company_address" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white" placeholder=""
                             value="{{old('company_address',isset($product) ? $product->company_address :'')}}"
                             required>
                    ...
                    <div class="grid grid-cols-1 gap-6 2xl:grid-cols-2">
                      <div class="flex flex-col mb-3  gap-2">
                        ...
                        <input type="text" name="company_director" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white" placeholder=""
                               value="{{old('company_director',isset($product) ? $product->company_director :'')}}"
                               required>
                      ...
                      <div class="flex flex-col mb-3  gap-2">
                        ...
                        <input type="text" name="company_accountant" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white" placeholder=""
                               value="{{old('company_accountant',isset($product) ? $product->company_accountant :'')}}"
                               required>
                  ...
                  <div>
                    ...
                    <div class="grid grid-cols-1 gap-6 2xl:grid-cols-2">
                      ...
                      <div class="flex flex-col mt-3 mb-3 gap-2">
                        ...
                        <input type="text" name="partner_company_name" id="partner_company_name" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white" placeholder=""
                               value="{{old('partner_company_name',isset($product) ? $product->partner_company_name :'')}}" required>
                    ...
                    <div class="grid grid-cols-1 gap-6 2xl:grid-cols-2">
                      <div class="flex flex-col mb-3  gap-2">
                        ...
                        <input type="text" name="partner_bank_code" id="partner_bank_code" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white" placeholder=""
                               value="{{old('partner_bank_code',isset($product) ? $product->partner_bank_code :'')}}" required>
                      ...
                      <div class="flex flex-col mb-3  gap-2">
                        ...
                        <input type="text" name="partner_mfo" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white" placeholder=""
                               value="{{old('partner_mfo',isset($product) ? $product->partner_mfo :'')}}" required>
                        @error('partner_mfo')
                        <small class="invalid-feedback"> {{ $message }} </small>
                        @enderror
                      </div>
                    ...
                    <div class="flex flex-col mb-3  gap-2">
                      ...
                      <input type="text" name="partner_address" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white" placeholder=""
                             value="{{old('partner_address',isset($product) ? $product->partner_address :'')}}"
                             required>
                    ...
                    <div class="grid grid-cols-1 gap-6 2xl:grid-cols-2">
                      <div class="flex flex-col mb-3  gap-2">
                        ...
                        <input type="text" name="partner_director" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white" placeholder=""
                               value="{{old('partner_director',isset($product) ? $product->partner_director :'')}}"
                               required>
                      ...
                      <div class="flex flex-col mb-3  gap-2">
                        ...
                        <input type="text" name="partner_accountant" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white" placeholder=""
                               value="{{old('partner_accountant',isset($product) ? $product->partner_accountant :'')}}"
                               required>
              ...
              @if(!isset($product) || $product->owner==\App\Services\DidoxService::OWNER_TYPE_OUTGOING)
                ...
                <div class="modal-content">
                  <div class="product-inputs">
                    ...
                    <div class="grid grid-cols-1 gap-6 2xl:grid-cols-2">
                      ...
                      <div style="margin-bottom: 15px;" class="flex flex-col gap-2">
                        <label class="text-base font-medium text-bgray-600 dark:text-bgray-50">{{__('main.product_origin')}}</label>
    ...
    @section('js')
      <script>
          $(document).ready(function () {
                  ...
                  $.ajax({
                      ...
                      success: function ($response) {
                          if ($response.status) {
                            ...
                            @isset($product)
                            $('#contract_id option[value={{$product->contract_id}}]').attr('selected', 'selected');
                            @endisset
                          } else {
                              alert($response.error);
                          }
            ...
            @if(!isset($product) && $current_company)
