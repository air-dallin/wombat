...
@endempty
@section('title')
  <h3 class="text-xl font-bold text-bgray-900 dark:text-white sm:text-2xl">АКТ ВЫПОЛНЕННЫХ РАБОТ<br>№ {{$act->number}} от {{$act->date}}</h3>
@endsection
...
@empty($openai)
  ...
  <div class="flex h-[56px] w-full space-x-4 mb-8">
    ...
    @if($act->status!=\App\Services\DidoxService::STATUS_DELETED)
      <div class="flex items-center sm:w-20 lg:w-88">
        <a href="{{localeRoute('frontend.profile.modules.act.check-status',$act)}}" class="flex h-full w-full items-center justify-center rounded-lg border border-bgray-300  bg-warning-300 text-white transition duration-300 ease-in-out hover:bg-success-400" title="{{__('main.check_status')}}" style="max-width: 200px;">
          {{__('main.check_status')}}
        </a>
        <span class="inline-flex items-center justify-center gap-x-2 rounded-lg border border-transparent bg-white p-4 text-base text-bgray-600 transition duration-300 ease-in-out hover:border-success-300 dark:bg-darkblack-600 dark:text-white" style="margin-left: 15px"> {!! \App\Services\DidoxService::getStatusLabel($act->doc_status) !!}{{ \App\Services\DidoxService::getStatus($act->doc_status) }}</span>
      </div>
      ...
      <div class="relative h-full">
        <div class="flex h-full  space-x-4">
          ...
          @elseif(!empty($act->response_sign) && !empty($sign))
                {{--@if($act->doc_status!=\App\Services\DidoxService::STATUS_SIGNED)--}}
                    @if($act->owner==\App\Services\DidoxService::OWNER_TYPE_INCOMING)
          ...
          <a href="{{localeRoute('frontend.profile.modules.act.print',$act->id)}}" class="inline-flex items-center justify-center gap-x-2 rounded-lg border border-transparent bg-white p-4 text-base text-bgray-600 transition duration-300 ease-in-out hover:border-success-300 dark:bg-darkblack-600 dark:text-white" target="_blank">
          ...
          <a href="{{localeRoute('frontend.profile.modules.act.download',$act->id)}}" target="_blank" class="inline-flex items-center justify-center gap-x-2 rounded-lg border border-transparent bg-white p-4 text-base text-bgray-600 transition duration-300 ease-in-out hover:border-success-300 dark:bg-darkblack-600 dark:text-white" style="min-width: 118px;">
          ...
          {{--              <a href="{{localeRoute('frontend.profile.modules.act.download',$act->id)}}" target="_blank" class="flex h-full w-full items-center justify-center rounded-lg border border-bgray-300 bg-bgray-100 dark:border-darkblack-500 dark:bg-darkblack-500" style="border: 1px solid #718096; color: #718096; min-width: 200px;">{{__('main.download_archiev')}}</a>--}}
          {{--              <a href="{{localeRoute('frontend.profile.modules.act.exportPdf',$act->id)}}" type="button" class="flex h-full w-full items-center justify-center rounded-lg border border-bgray-300 bg-bgray-100 dark:border-darkblack-500 dark:bg-darkblack-500" target="_blank" style="border: 1px solid #718096; color: #718096; min-width: 200px;">{{__('main.download_pdf')}}</a>--}}
          {{--              <a href="{{localeRoute('frontend.profile.modules.act.print',$act->id)}}" type="button" class="flex h-full w-full items-center justify-center rounded-lg border border-bgray-300 bg-bgray-100 dark:border-darkblack-500 dark:bg-darkblack-500" target="_blank" style="border: 1px solid #718096; color: #718096; min-width: 200px;">{{__('main.print')}}</a>--}}
          {{--      @if($act->status!=\App\Services\DidoxService::STATUS_DELETED)
                  @if($act->isNotSigned())
                    ...
                    <div id="document-json" style="display:none">{{$document}}</div>
                  @elseif(!empty($act->response_sign))
                  ...
                  <a href="{{localeRoute('frontend.profile.modules.act.check-status',$act)}}" class="flex h-full w-full items-center justify-center rounded-lg border border-bgray-300 bg-bgray-100 dark:border-darkblack-500 dark:bg-darkblack-500 btn-icon" title="{{__('main.update_from_didox')}}" style="border: 1px solid #718096; color: #718096; min-width: 100px;"><i class="fa fa-download"></i></a>
                @else
    ...
    @else
      <div class="sm:w-20 lg:w-88">
        {!! \App\Services\DidoxService::getStatusLabel($act->doc_status) !!}{{ \App\Services\DidoxService::getStatus($act->doc_status) }}</span>
      </div>
        ...
        <div class="modal-body">
          <form name="eimzo">
            ...
            <div style="display: none;">
              <input type="text" name="document_id" value="{{$act->id}}">
              <input type="text" name="object" value="Act">
              <input type="text" name="company_id" id="company_id" value="{{$act->company_id}}">
              <input type="text" id="hex" value="">
              <input type="text" id="message" value="">
              <input type="text" id="serial" value="">
              <input type="text" id="status" value="{{$act->doc_status}}">
              <input type="text" id="inn" value="{{base64_encode($act->company->inn)}}">
              <input type="text" id="tokenExpire" value="{{$act->company->checkTokenExpire()}}">
