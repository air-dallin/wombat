...
@section('title')
  ...
  @include('frontend.profile.sections.document_create_menu',['currentMenuType'=>'act'])
        ...
        <div class="card">
          <div class="card-body">
            <form method="POST" id="form_act"
                  @isset($act)
                    action="{{localeRoute('frontend.profile.modules.act.update',$act)}}"
                  @else
                    action="{{localeRoute('frontend.profile.modules.act.store')}}"
                @endisset
            >
              @csrf
              ...
              <div class="p-5 mb-5 rounded-lg bg-white dark:bg-darkblack-600">
                    ...
                    @isset($plans)
                      @foreach($plans as $plan)
                        <option value="{{$plan->id}}"
                                @if((isset($act) && $act->plan_id==$plan->id) || (!empty($selected_plan) && $selected_plan->plan_id==$plan->id)) selected @endif >{{$plan->code . ' - ' . $plan->title_ru }}
                        </option>
                      @endforeach
                    @endisset
                  </select>
                ...
                <div class="grid grid-cols-1 gap-6 2xl:grid-cols-2">
                  ...
                  <div class="flex flex-col gap-2">
                    ...
                    <input type="text" name="number" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white @error('number') is-invalid @enderror @error('number') is-invalid @enderror" placeholder="" value="{{old('number',isset($act) ? $act->number : $new_number)}}" required>
                    @error('number')
                    <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                  ...
                  <div class="flex flex-col gap-2">
                    ...
                    <input type="date" name="date" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white @error('date') is-invalid @enderror" placeholder="" value="{{old('date',isset($act) ? $act->date :date('Y-m-d'))}}" required>
                    @error('date')
                    <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                  ...
                  {{--<div class="flex flex-col gap-2">
                    <label for="" class="text-base font-medium text-bgray-600 dark:text-bgray-50">{{__('main.act_expire')}}</label>
                    <input type="date" name="act_expire" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white @error('act_expire') is-invalid @enderror" placeholder="" value="{{old('act_expire',isset($act) ? $act->act_expire :date('Y-m-d'))}}" required>
                    @error('act_expire')
                    <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                  ...
                  <div class="flex flex-col gap-2">
                    <label for="" class="text-base font-medium text-bgray-600 dark:text-bgray-50">{{__('main.act_place')}}</label>
                    <input type="text" name="act_place" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white @error('act_place') is-invalid @enderror" placeholder="" value="{{old('act_place',isset($act) ? $act->act_place :'')}}" required>
                    @error('act_place')
                    <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                    ...
                    <div class="grid grid-cols-1 gap-6 2xl:grid-cols-2">
                      <div class="flex flex-col mt-3 mb-3 gap-2">
                        ...
                        <input type="text" name="company_inn"
                               ...
                               placeholder=""
                               value="{{old('company_inn',isset($act) ? $act->company_inn :'')}}">
                        @error('company_inn')
                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                      ...
                      <div class="flex flex-col mt-3 mb-3 gap-2">
                        ...
                        <select class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white" name="company_id" id="company_id" required>
                          ...
                          @foreach($companies as $_company)
                            <option value="{{$_company->id}}" @if( (isset($act) && $act->company_id==$_company->id) || $_company->id==$current_company) selected @endif >{{$_company->name}}</option>
                          @endforeach
                        </select>
                        @error('company_id')
                        <span class="invalid-feedback" role="alert">
                      ...
                      <div class="flex flex-col mb-3 gap-2">
                          ...
                          <select class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white @error('company_invoice_id') is-invalid @enderror"
                            ...
                            @isset($act->company->invoices)
                              @foreach($act->company->invoices as $invoice)
                                <option value="{{$invoice->id}}"
                                        @if(isset($act) && $act->company_account==$invoice->bank_invoice) selected @endif >{{$invoice->bank_invoice}}</option>
                                        --}}{{--@if(isset($act) && $act->company_invoice_id==$invoice->id) selected @endif >{{$invoice->bank_invoice}}</option>--}}{{--
                              @endforeach
                            @endisset
                          </select>
                        ...
                        <div class="flex flex-col mb-3 gap-2">
                          ...
                          <input type="text" name="company_mfo" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white" placeholder=""
                                 value="{{old('company_mfo',isset($act) ? $act->company_mfo :'')}}" required>
                          @error('company_mfo')
                      ...
                      <div class="flex flex-col mb-3 gap-2">
                        ...
                        <input type="text" name="company_address" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white" placeholder=""
                               value="{{old('company_address',isset($act) ? $act->company_address :'')}}"
                               required>
                      ...
                      <div class="grid grid-cols-1 gap-6 2xl:grid-cols-2">
                        <div class="flex flex-col mb-3 gap-2">
                          ...
                          <input type="text" name="company_director" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white" placeholder=""
                                 value="{{old('company_director',isset($act) ? $act->company_director :'')}}"
                                 required>
                        ...
                        <div class="flex flex-col mb-3 gap-2">
                          ...
                          <input type="text" name="company_accountant" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white" placeholder=""
                                 value="{{old('company_accountant',isset($act) ? $act->company_accountant :'')}}"
                                 required>
                    ...
                    <div>
                      ...
                      <div class="grid grid-cols-1 gap-6 2xl:grid-cols-2">
                        ...
                        <div class="flex flex-col mt-3 mb-3 gap-2">
                          ...
                          <input type="text" name="partner_company_name" id="partner_company_name" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white" placeholder=""
                                 value="{{old('partner_company_name',isset($act) ? $act->partner_company_name :'')}}" required>
                      ...
                      <div class="grid grid-cols-1 gap-6 2xl:grid-cols-2">
                        <div class="flex flex-col mb-3 gap-2">
                          ...
                          <input type="text" name="partner_bank_code" id="partner_bank_code" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white" placeholder=""
                                 value="{{old('partner_bank_code',isset($act) ? $act->partner_bank_code :'')}}" required>
                        ...
                        <div class="flex flex-col mb-3 gap-2">
                          ...
                          <input type="text" name="partner_mfo" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white" placeholder=""
                                 value="{{old('partner_mfo',isset($act) ? $act->partner_mfo :'')}}" required>
                          @error('partner_mfo')
                          <small class="invalid-feedback"> {{ $message }} </small>
                          @enderror
                        </div>
                      ...
                      <div class="flex flex-col mb-3 gap-2">
                        ...
                        <input type="text" name="partner_address" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white" placeholder=""
                               value="{{old('partner_address',isset($act) ? $act->partner_address :'')}}"
                               required>
                      ...
                      <div class="grid grid-cols-1 gap-6 2xl:grid-cols-2">
                        <div class="flex flex-col mb-3 gap-2">
                          ...
                          <input type="text" name="partner_director" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white" placeholder=""
                                 value="{{old('partner_director',isset($act) ? $act->partner_director :'')}}"
                                 required>
                        ...
                        <div class="flex flex-col mb-3 gap-2">
                          ...
                          <input type="text" name="partner_accountant" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white" placeholder=""
                                 value="{{old('partner_accountant',isset($act) ? $act->partner_accountant :'')}}"
                                 required>
              ...
              <div class="p-5 mb-5 rounded-lg bg-white dark:bg-darkblack-600">
               {{-- <div class="flex flex-col mb-3 gap-2">
                  <label class="text-base font-medium text-bgray-600 dark:text-bgray-50">{{__('main.act_name')}}</label>
                  <input type="text" name="act_name" id="act_name" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white @error('act_name') is-invalid @enderror" value="{{old('act_name',isset($act) ? $act->act_name :'')}}" required>
                  @error('act_name')
                  <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                ...
                <div class="flex flex-col gap-2">
                  <label class="text-base font-medium text-bgray-600 dark:text-bgray-50">{{__('main.act_text')}}</label>
                  <textarea class="h-34 rounded-lg border-0 bg-bgray-50 p-4 focus:border  focus:ring-0 dark:bg-darkblack-500 dark:text-white" name="act_text" rows="6" required>{{old('act_text',isset($act) ? $act->act_text :'')}}</textarea>
                  @error('act_text')
                  <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
              ...
              @if(!isset($act) || $act->owner==\App\Services\DidoxService::OWNER_TYPE_OUTGOING)
  ...
  <script>
      $(document).ready(function () {
          ...
          $('.btn-create').click(function (e) {
              ...
              //if(!$(e.target).parents('#form_act').isValid()) e.preventDefault();
              $('#form_act').submit();
          });
        ...
        @if(!isset($act) && $current_company) /*$current_company)*/
