            ...
            @isset($expense_orders)
                @foreach($expense_orders as $expense_order)
                    <tr class="border-b border-bgray-300 dark:border-darkblack-400">
                        ...
                        <td class="px-6 py-5">@isset($expense_order->contract)
                                {{$expense_order->contract->contract_number . ' - ' . $expense_order->contract->contract_date }}
                            @endisset</td>
                                ...
                                <div id="cardsOptions{{$expense_order->id}}" class="rounded-lg shadow-lg min-w-[100px] bg-white dark:bg-darkblack-500 absolute right-10 z-10 top-full hidden overflow-hidden" style="display: none;">
                                    <ul style="min-width: 100px; text-align: center">
                                        <li class="text-sm text-bgray-900 cursor-pointer px-5 py-2 hover:bg-bgray-100 hover:dark:bg-darkblack-600 dark:text-white font-semibold">
                                            <a href="{{localeRoute('frontend.profile.modules.incoming_order.edit',$expense_order)}}" class="inline-flex h-8 w-8 translate-y-0 transform items-center justify-center transition duration-300 ease-in-out hover:-translate-y-1">
                                                {{ __('main.edit') }}
                                            </a>
                                        </li>
                                    </ul>
