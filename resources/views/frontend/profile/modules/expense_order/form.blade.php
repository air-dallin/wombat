...
@section('title',__('main.expense_orders'))
@section('content')
      ...
      <div class="col-12">
        <div class="card">
          ...
          <div class="card-body">
            <form method="POST"
                  @isset($expense_order)
                    action="{{localeRoute('frontend.profile.modules.expense_order.update',$expense_order)}}"
                  @else
                    action="{{localeRoute('frontend.profile.modules.expense_order.store')}}"
                @endisset
            >
              @csrf
              <div class="grid grid-cols-1 gap-6 2xl:grid-cols-2">
                <div class="flex flex-col gap-2">
                  ...
                  <select class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border focus:ring-0 dark:bg-darkblack-500 dark:text-white @error('contract_id') is-invalid @enderror" name="contract_id" id="contract_id" required>
                    ...
                    @foreach($contracts as $contract)
                      <option value="{{$contract->id}}" data-date="{{ $contract->contract_date }}" @if(isset($expense_order) && $expense_order->contract_id==$contract->id) selected @endif >{{ $contract->contract_number . ' - ' . $contract->contract_date }}</option>
                    @endforeach
                  </select>
                  @error('contract_id')
                ...
                <div class="flex flex-col gap-2">
                  ...
                  <input type="date" name="order_date" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border focus:ring-0 dark:bg-darkblack-500 dark:text-white @error('order_date') is-invalid @enderror" placeholder="" value="{{old('order_date',isset($expense_order) ? $expense_order->order_date :date('Y-m-d'))}}" required>
                  @error('order_date')
                  <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                ...
                <div class="flex flex-col gap-2">
                  ...
                  <input type="text" name="contragent_company" id="contragent_company" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border focus:ring-0 dark:bg-darkblack-500 dark:text-white @error('contragent_company') is-invalid @enderror" value="{{old('contragent_company',isset($expense_order) ? $expense_order->contragent_company :'')}}" required>
                ...
                <div class="flex flex-col gap-2">
                  ...
                  <select class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border focus:ring-0 dark:bg-darkblack-500 dark:text-white @error('casse_id') is-invalid @enderror" name="casse_id" id="casse_id" required>
                    <option value="">{{__('main.choice_casse')}}</option>
                    @isset($expense_order)
                      @foreach($company_casses as $case)
                        <option value="{{$case->id}}" @if(isset($expense_order) && $expense_order->casse_id==$case->id) selected @endif >{{$case->getTitle()}}</option>
                      @endforeach
                    @endisset
                  </select>
                  @error('casse_id')
                ...
                <div class="flex flex-col gap-2">
                  ...
                  <input type="text" name="amount" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border focus:ring-0 dark:bg-darkblack-500 dark:text-white @error('amount') is-invalid @enderror" placeholder="" value="{{old('amount',isset($expense_order) ? $expense_order->amount :'')}}" required>
                  @error('amount')
                  <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                ...
                <div class="flex flex-col gap-2">
                  ...
                  <select class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border focus:ring-0 dark:bg-darkblack-500 dark:text-white @error('choice_movement') is-invalid @enderror" name="movement_id" required>
                    ...
                    @foreach($movements as $movement)
                      <option value="{{$movement->id}}" @if(isset($expense_order) && $expense_order->movement_id==$movement->id) selected @endif >{{$movement->getTitle()}}</option>
                    @endforeach
                  </select>
                  @error('movement_id')
                  <span class="invalid-feedback" role="alert">
                ...
                <div class="flex flex-col gap-2">
                  ...
                  <select class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border focus:ring-0 dark:bg-darkblack-500 dark:text-white" name="status">
                    <option value="0" @if(isset($expense_order) && $expense_order->status==0) selected @endif >{{__('main.draft')}}</option>
                    <option value="1" @if(isset($expense_order) && $expense_order->status==1) selected @endif >{{__('main.active')}}</option>
                  </select>
                </div>
              </div>
              <div class="flex flex-col mt-5">
                <label class="text-base font-medium text-bgray-600 dark:text-bgray-50">{{__('main.comment')}}</label>
                <textarea class="h-24 rounded-lg border-0 bg-bgray-50 p-4 focus:border focus:ring-0 dark:bg-darkblack-500 dark:text-white" name="comment" rows="6">{{old('comment',isset($expense_order) ? $expense_order->comment :'')}}</textarea>
                @error('comment')
                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
              ...
              <div class="flex justify-end">
                @isset($expense_order)
                  <a href="{{localeRoute('frontend.profile.modules.expense_order.print',$expense_order)}}" target="_blank" class="btn-create btn btn-info" title="{{__('main.print')}}"><i class="fa fa-print"></i></a>
                  <a href="{{localeRoute('frontend.profile.modules.expense_order.exportPdf',$expense_order)}}" target="_blank" class="btn-create btn btn-info" title="{{__('main.pdf')}}"><i class="fa fa-file-pdf"></i></a>
                @endif
  ...
  <script>
      $(document).ready(function () {
          ...
          $('#company_id').change(function () {
              ...
              $.ajax({
                  type: 'post',
                  url: '/ru/profile/modules/expense_order/cases',
