    ...
    <div class="col-12">
      <div class="card">
        <div class="card-header justify-content-between">
              ...
              <div class="relative h-full">
                @if(true || (!empty($tarif) && $tarif->checkTarifIsActive()))
                  <div class="flex h-full  space-x-4">
                    <a href="{{ localeRoute("frontend.profile.modules.expense_order.index") }}" class="flex h-full w-full items-center justify-center rounded-lg border border-bgray-300 bg-bgray-100 dark:border-darkblack-500 dark:bg-darkblack-500" style="border: 1px solid #718096; color: #718096; min-width: 200px;">{{__('main.expense_orders')}}</a>
                    <a href="{{ localeRoute('frontend.profile.modules.incoming_order.draft') }}" class="flex h-full w-full items-center justify-center rounded-lg border border-bgray-300 bg-bgray-100 dark:border-darkblack-500 dark:bg-darkblack-500 {{App\Helpers\MenuHelper::check($controller.'.'.$action,'incomingorder.draft')}}" style="border: 1px solid #ffa500; color: #ffa500; min-width: 200px;">{{__('main.draft')}}</a>
                    <a href="{{ localeRoute('frontend.profile.modules.incoming_order.create') }}" class="flex h-full w-full items-center justify-center rounded-lg border border-bgray-300  bg-success-300 text-white transition duration-300 ease-in-out hover:bg-success-400" style="min-width: 200px;">{{__('main.create')}}</a>
                  </div>
                @endif
              </div>
            </div>
          ...
          @include('frontend.profile.modules.incoming_order.table')
