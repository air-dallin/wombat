...
@section('content')
  ...
  <div class="grid grid-cols-1 rounded-xl bg-white dark:bg-darkblack-600 xl:grid-cols-12">
    ...
    <!-- Sidebar Tabs -->
    @include('frontend.profile.sections.company_menu',['menu'=>'company_ikpu'])
    ...
    <div class="tab-content col-span-9 px-10 py-8">
      ...
      <div id="tab4" class="tab-pane active">
        <div class="border-b border-bgray-200 pb-5 dark:border-darkblack-400 flex justify-between items-center">
          <h3 class=" text-2xl font-bold text-bgray-900 dark:text-white">{{ __('main.company_ikpus') }} </h3>
          <a href="{{ localeRoute('frontend.profile.company_ikpu.create') }}" class="rounded-lg bg-success-300 px-4 py-3.5 font-semibold text-white">{{__('main.create')}}</a>
        </div>
        <table class="w-full">
          <tbody>
          <tr class="border-b border-bgray-300 dark:border-darkblack-400">
            <td class="sorting text-base font-medium text-bgray-600 dark:text-bgray-50 px-6 py-5 xl:px-0" data-sort="status">{{__('main.company_name')}} {!! \App\Helpers\QueryHelper::getDirectionLabel('company_id') !!}</td>
            <td class="sorting text-base font-medium text-bgray-600 dark:text-bgray-50" data-sort="name">{{__('main.ikpu')}} {!! \App\Helpers\QueryHelper::getDirectionLabel('ikpu_id') !!}</td>
          ...
          @foreach($companyIkpus as $ikpu)
            <tr class="border-b border-bgray-300 dark:border-darkblack-400">
              <td class="px-6 py-5 xl:px-0">{{ $ikpu->company->name  }}</td>
              <td class="px-6 py-5 xl:px-0">{{ \Illuminate\Support\Str::limit($ikpu->getTitle(), 17, '')}}</td>
              <td class="px-6 py-5 xl:px-0">{{date('Y-m-d H:i',strtotime($ikpu->created_at))}}</td>
                  ...
                  <div id="cardsOptions{{$ikpu->id}}" class="rounded-lg shadow-lg min-w-[100px] bg-white dark:bg-darkblack-500 absolute right-10 z-10 top-full hidden overflow-hidden" style="display: none;">
                    <ul style="min-width: 100px; text-align: center">
                      <li class="text-sm text-bgray-900 cursor-pointer px-5 py-2 hover:bg-bgray-100 hover:dark:bg-darkblack-600 dark:text-white font-semibold">
                        <a href="{{localeRoute('frontend.profile.company_ikpu.edit',$ikpu)}}" class="inline-flex h-8 w-8 translate-y-0 transform items-center justify-center transition duration-300 ease-in-out hover:-translate-y-1">
                          {{ __('main.edit') }}
                        </a>
                      </li>
                      <li class="text-sm text-bgray-900 cursor-pointer px-5 py-2 hover:bg-bgray-100 hover:dark:bg-darkblack-600 dark:text-white font-semibold">
