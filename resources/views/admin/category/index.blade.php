    ...
    <div class="col-12">
      <div class="card">
        <div class="card-body">
          <div class="table-responsive">
            <table class="w-full">
              ...
              <tr class="border-b border-bgray-300 dark:border-darkblack-400">
                ...
                <th class="sorting text-base font-medium text-bgray-600 dark:text-bgray-50 px-6 py-5" data-sort="title_{{app()->getLocale()}}"><strong>Title</strong> {!! \App\Helpers\QueryHelper::getDirectionLabel('title_'.app()->getLocale()) !!}</th>
                <th class="text-base font-medium text-bgray-600 dark:text-bgray-50 px-6 py-5"></th>
              </tr>
              @if($categories)
                @foreach($categories as $category)
                  <tr class="border-b border-bgray-300 dark:border-darkblack-400 hover:bg-gray-300 clickable-row">
                    ...
                    <td class="px-6 py-5">{{ $category->getTitle() }}  </td>
                    <td class="px-6 py-5">
                      <div class="payment-select relative">
                        <button onclick="dateFilterAction('#cardsOptions{{$category->id}}')" type="button">
                        ...
                        <div id="cardsOptions{{$category->id}}" class="rounded-lg shadow-lg min-w-[100px] bg-white dark:bg-darkblack-500 absolute right-10 z-10 top-full hidden overflow-hidden" style="display: none;">
                          <ul style="min-width: 100px; text-align: center">
                            <li class="text-sm text-bgray-900 cursor-pointer px-5 py-2 hover:bg-bgray-100 hover:dark:bg-darkblack-600 dark:text-white font-semibold">
                              <a href="{{localeRoute('admin.category.edit',$category)}}" class="inline-flex h-8 w-8 translate-y-0 transform items-center justify-center transition duration-300 ease-in-out hover:-translate-y-1">
                                {{ __('main.edit') }}
                              </a>
                            </li>
                            <li class="text-sm text-bgray-900 cursor-pointer px-5 py-2 hover:bg-bgray-100 hover:dark:bg-darkblack-600 dark:text-white font-semibold">
                              <form action="{{localeRoute('admin.category.destroy',$category)}}}" method="POST">
                                @csrf
                                @method('PUT')
          ...
          {{ $categories->onEachSide(3)->withQueryString()->links('frontend.profile.sections.pagination') }}
