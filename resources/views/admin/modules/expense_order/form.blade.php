...
@section('title', __('main.expense_order'))
...
@section('content')
  ...
  /* <div class="page-titles">
             <ol class="breadcrumb">
                 ...
                 <li class="breadcrumb-item active"><a href="javascript:void(0)">{{isset($expense_order)?$expense_order->title_ru :'' }}</a></li>
             </ol>
         </div> */ ?>
    ...
    <div class="col-12">
      ...
      <div class="card">
        ...
        <div class="card-body">
          <form method="POST"
                @isset($expense_order)
                  action="{{localeRoute('admin.modules.expense_order.update',$expense_order)}}"
                @else
                  action="{{localeRoute('admin.modules.expense_order.store')}}"
              @endisset
          >
            @csrf
            <div class="grid grid-cols-2 gap-6 2xl:grid-cols-3 mb-3">
              <div class="flex flex-col gap-2">
                ...
                <input type="text" name="contract_number" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border focus:ring-0 dark:bg-darkblack-500 dark:text-white  @error('contract_number') is-invalid @enderror @error('contract_number') is-invalid @enderror" placeholder="" value="{{old('contract_number',isset($expense_order) ? $expense_order->contract_number :'')}}" required>
              ...
              <div class="flex flex-col gap-2">
                ...
                <input type="date" name="contract_date" class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border focus:ring-0 dark:bg-darkblack-500 dark:text-white  @error('contract_date') is-invalid @enderror" placeholder="" value="{{old('contract_date',isset($expense_order) ? $expense_order->contract_date :'')}}" required>
                @error('contract_date')
                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
            ...
            <div class="grid grid-cols-2 gap-6 2xl:grid-cols-4 mb-3">
              ...
              <div class="flex flex-col gap-2">
                ...
                <select class="h-14 rounded-lg border-0 bg-bgray-50 p-4 focus:border focus:ring-0 dark:bg-darkblack-500 dark:text-white " name="status">
                  <option value="0" @if(isset($expense_order) && $expense_order->status==0) selected @endif >{{__('main.inactive')}}</option>
                  <option value="2" @if(isset($expense_order) && $expense_order->status==2) selected @endif >{{__('main.draft')}}</option>
