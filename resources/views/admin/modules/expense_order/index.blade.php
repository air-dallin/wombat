        ...
        <div class="card-header justify-content-between">
          ...
          {{--           <div>
                        @if($tarif->checkTarifIsActive())
                             <a href="{{ localeRoute('admin.modules.expense_order.create') }}" class="btn-create btn btn-success">{{__('main.create')}}</a>
                        @endif
                         <a href="{{ localeRoute('admin.modules.expense_order.draft') }}" class="btn-create btn btn-success">{{__('main.draft')}}</a>
                     </div>--}}
        </div>
        <div class="card-body">
          <div class="tickets-table table-responsive">
            <table class="w-full">
              ...
              @if($expense_orders)
                @foreach($expense_orders as $expense_order)
                  <tr class="border-b border-bgray-300 dark:border-darkblack-400 hover:bg-gray-300 clickable-row">
                    <td class="px-6 py-5">{!! \App\Models\Module::getStatusLabel($expense_order->status) !!}</td>
                    <td class="px-6 py-5">{{$expense_order->contract_number}}</td>
                    <td class="px-6 py-5">{{$expense_order->amount}}</td>
                    <td class="px-6 py-5">{{$expense_order->casse->getTitle()}}</td>
                    <td class="px-6 py-5">{{$expense_order->movement->getTitle()}}</td>
                    <td class="px-6 py-5">{{$expense_order->contragent}}</td>
                    <td class="px-6 py-5">{{date('Y-m-d H:i',strtotime($expense_order->contract_date))}}</td>
                    <td class="px-6 py-5">{{date('Y-m-d H:i',strtotime($expense_order->order_date))}}</td>
                    <td class="px-6 py-5">{{date('Y-m-d H:i',strtotime($expense_order->created_at))}}</td>
                    <td class="px-6 py-5">
                      <div class="payment-select relative">
                        <button onclick="dateFilterAction('#cardsOptions{{$expense_order->id}}')" type="button">
                        ...
                        <div id="cardsOptions{{$expense_order->id}}" class="rounded-lg shadow-lg min-w-[100px] bg-white dark:bg-darkblack-500 absolute right-10 z-10 top-full hidden overflow-hidden" style="display: none;">
                          <ul style="min-width: 100px; text-align: center">
                            <li class="text-sm text-bgray-900 cursor-pointer px-5 py-2 hover:bg-bgray-100 hover:dark:bg-darkblack-600 dark:text-white font-semibold">
                              <a href="{{localeRoute('admin.expense_order.edit',$expense_order)}}" class="inline-flex h-8 w-8 translate-y-0 transform items-center justify-center transition duration-300 ease-in-out hover:-translate-y-1">
                                {{ __('main.edit') }}
                              </a>
                            </li>
                            {{--                                                        <li class="text-sm text-bgray-900 cursor-pointer px-5 py-2 hover:bg-bgray-100 hover:dark:bg-darkblack-600 dark:text-white font-semibold">--}}
                            {{--                                                            <form action="{{localeRoute('admin.expense_order.destroy',$expense_order)}}}" method="POST">--}}
                            {{--                                                                @csrf--}}
                            {{--                                                                @method('PUT')--}}
            ...
            <div class="mt-4">
              {{ $expense_orders->onEachSide(3)->withQueryString()->links('frontend.profile.sections.pagination') }}
            </div>
          </div>
        </div>
      </div>
