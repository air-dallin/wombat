...
@section('title', __('main.purchase_invoices'))
            ...
            <table class="w-full">
              ...
              <tr class="border-b border-bgray-300 dark:border-darkblack-400">
                <td class="sorting text-base font-medium text-bgray-600 dark:text-bgray-50 px-6 py-5"
                    data-sort="contract_id">{{__('main.contract')}} {!! \App\Helpers\QueryHelper::getDirectionLabel('contract_id') !!}</td>
                {{--<td class="sorting text-base font-medium text-bgray-600 dark:text-bgray-50 px-6 py-5"
                ...
                <td class="sorting text-base font-medium text-bgray-600 dark:text-bgray-50 px-6 py-5"
                    data-sort="company">{{__('main.company')}} {!! \App\Helpers\QueryHelper::getDirectionLabel('company') !!}</td>
                <td class="sorting text-base font-medium text-bgray-600 dark:text-bgray-50 px-6 py-5"
              ...
              @if($products)
                @foreach($products as $product)
                  <tr class="border-b border-bgray-300 dark:border-darkblack-400 hover:bg-gray-300 clickable-row">
                    <td class="px-6 py-5">{{@$product->contract->contract_number }}</td>
                    <td class="px-6 py-5">{{$product->company->name}}</td>
                    <td class="px-6 py-5">{{$product->contragent}}</td>
                    <td class="px-6 py-5">{{$product->contragent_company}}</td>
                    <td class="px-6 py-5">{!! \App\Models\Module::getStatusLabel($product->status) !!}</td>
                    <td class="px-6 py-5">{{date('Y-m-d H:i',strtotime($product->created_at))}}</td>
                    <td class="px-6 py-5">
                      <div class="payment-select relative">
                        <button onclick="dateFilterAction('#cardsOptions{{$product->id}}')" type="button">
                        ...
                        <div id="cardsOptions{{$product->id}}" class="rounded-lg shadow-lg min-w-[100px] bg-white dark:bg-darkblack-500 absolute right-10 z-10 top-full hidden overflow-hidden" style="display: none;">
                          <ul style="min-width: 100px; text-align: center">
                            <li class="text-sm text-bgray-900 cursor-pointer px-5 py-2 hover:bg-bgray-100 hover:dark:bg-darkblack-600 dark:text-white font-semibold">
                              <a href="{{localeRoute('admin.modules.product.edit',$product)}}" class="inline-flex h-8 w-8 translate-y-0 transform items-center justify-center transition duration-300 ease-in-out hover:-translate-y-1">
                                {{ __('main.edit') }}
                              </a>
                            </li>
                            {{--                                                        <li class="text-sm text-bgray-900 cursor-pointer px-5 py-2 hover:bg-bgray-100 hover:dark:bg-darkblack-600 dark:text-white font-semibold">--}}
                            {{--                                                            <form action="{{localeRoute('admin.modules.product.destroy',$product)}}}" method="POST">--}}
                            {{--                                                                @csrf--}}
                            {{--                                                                @method('PUT')--}}
