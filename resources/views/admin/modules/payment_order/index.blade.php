            ...
            <table class="w-full">
              ...
              <tr class="border-b border-bgray-300 dark:border-darkblack-400">
                ...
                <td class="sorting text-base font-medium text-bgray-600 dark:text-bgray-50 px-6 py-5"
                    data-sort="company_id">{{__('main.company')}} {!! \App\Helpers\QueryHelper::getDirectionLabel('company_id') !!}</td>
                <td class="sorting text-base font-medium text-bgray-600 dark:text-bgray-50 px-6 py-5"
                ...
                <td class="sorting text-base font-medium text-bgray-600 dark:text-bgray-50 px-6 py-5"
                    data-sort="region">{{__('main.invoice')}} {!! \App\Helpers\QueryHelper::getDirectionLabel('invoice_id') !!}</td>
                <td class="sorting text-base font-medium text-bgray-600 dark:text-bgray-50 px-6 py-5"
                    data-sort="region">{{__('main.contragent')}} {!! \App\Helpers\QueryHelper::getDirectionLabel('contragent') !!}</td>
                <td class="sorting text-base font-medium text-bgray-600 dark:text-bgray-50 px-6 py-5"
              ...
              @if($payment_orders)
                @foreach($payment_orders as $payment_order)
                  <tr class="border-b border-bgray-300 dark:border-darkblack-400 hover:bg-gray-300 clickable-row">
                    <td class="px-6 py-5">{!! \App\Models\Module::getStatusLabel($payment_order->status) !!}</td>
                    <td class="px-6 py-5">{{$payment_order->company->name}}</td>
                    <td class="px-6 py-5">{{$payment_order->contract_number}}</td>
                    <td class="px-6 py-5">{{$payment_order->amount}}</td>
                    <td class="px-6 py-5">{{$payment_order->invoice}}</td>
                    <td class="px-6 py-5">{{$payment_order->contragent}}</td>
                    <td class="px-6 py-5">{{date('Y-m-d H:i',strtotime($payment_order->contract_date))}}</td>
                        ...
                        <div id="cardsOptions{{$payment_order->id}}" class="rounded-lg shadow-lg min-w-[100px] bg-white dark:bg-darkblack-500 absolute right-10 z-10 top-full hidden overflow-hidden" style="display: none;">
                          <ul style="min-width: 100px; text-align: center">
                            <li class="text-sm text-bgray-900 cursor-pointer px-5 py-2 hover:bg-bgray-100 hover:dark:bg-darkblack-600 dark:text-white font-semibold">
                              <a href="{{localeRoute('admin.modules.payment_order.edit',$payment_order)}}" class="inline-flex h-8 w-8 translate-y-0 transform items-center justify-center transition duration-300 ease-in-out hover:-translate-y-1">
                                {{ __('main.edit') }}
                              </a>
                            </li>
                            {{--                                                        <li class="text-sm text-bgray-900 cursor-pointer px-5 py-2 hover:bg-bgray-100 hover:dark:bg-darkblack-600 dark:text-white font-semibold">--}}
                            {{--                                                            <form action="{{localeRoute('admin.payment_order.destroy',$payment_order)}}}" method="POST">--}}
                            {{--                                                                @csrf--}}
                            {{--                                                                @method('PUT')--}}
