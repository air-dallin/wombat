...
@section('title',__('main.plan') .' - ' . $company->name)
{{-- Content --}}
@section('content')
          ...
          <div class="table-responsive">
            <table class="w-full">
              ...
              <tr class="border-b border-bgray-300 dark:border-darkblack-400">
                ...
                <td class="-sorting text-base font-medium text-bgray-600 dark:text-bgray-50 px-6 py-5" data-sort="debit_account"><strong>{{__('main.debit_description')}}</strong> {!! \App\Helpers\QueryHelper::getDirectionLabel('debit_account_'.app()->getLocale()) !!}</td>
                ...
                <td class="-sorting text-base font-medium text-bgray-600 dark:text-bgray-50 px-6 py-5" data-sort="credit_account"><strong>{{__('main.credit_description')}}</strong> {!! \App\Helpers\QueryHelper::getDirectionLabel('credit_account') !!}</td>
              ...
              @if($accounts)
                @foreach($accounts as $account)
                  <tr class="border-b border-bgray-300 dark:border-darkblack-400 hover:bg-gray-300 clickable-row">
                    ...
                    <td class="px-6 py-5">{{ 'Оборудование' }}  </td>
                    <td class="px-6 py-5">{{ $account->credit_account }}  </td>
                    <td class="px-6 py-5">{{'Наличные' }}  </td>
