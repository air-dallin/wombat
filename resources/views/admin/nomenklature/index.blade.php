            ...
            <div class="table-responsive">
              <table class="w-full">
                ...
                <tr class="border-b border-bgray-300 dark:border-darkblack-400">
                  ...
                  <td class="sorting text-base font-medium text-bgray-600 dark:text-bgray-50 px-6 py-5" data-sort="company_id"><strong>{{__('main.company')}}</strong> {!! \App\Helpers\QueryHelper::getDirectionLabel('company_id') !!}</td>
                  <td class="sorting text-base font-medium text-bgray-600 dark:text-bgray-50 px-6 py-5" data-sort="ikpu_id"><strong>{{__('main.ikpu')}}</strong> {!! \App\Helpers\QueryHelper::getDirectionLabel('ikpu_id') !!}</td>
                  <td class="sorting text-base font-medium text-bgray-600 dark:text-bgray-50 px-6 py-5" data-sort="unit_id"><strong>{{__('main.unit')}}</strong> {!! \App\Helpers\QueryHelper::getDirectionLabel('unit_id') !!}</td>
                  ...
                  {{--  <td class="sorting" data-sort="category_id"><strong>{{__('main.category')}}</strong> {!! \App\Helpers\QueryHelper::getDirectionLabel('category_id') !!}</td>
                    <td class="sorting" data-sort="unit_id"><strong>{{__('main.unit')}}</strong> {!! \App\Helpers\QueryHelper::getDirectionLabel('unit_id') !!}</td>
                    <td class="sorting" data-sort="nds_id"><strong>{{__('main.nds')}}</strong> {!! \App\Helpers\QueryHelper::getDirectionLabel('nds_id') !!}</td>
                    <td class="sorting" data-sort="article"><strong>{{__('main.article')}}</strong> {!! \App\Helpers\QueryHelper::getDirectionLabel('article') !!}</td>
                   --}}
                          ...
                          <div id="cardsOptions{{$nomenklature->id}}" class="rounded-lg shadow-lg min-w-[100px] bg-white dark:bg-darkblack-500 absolute right-10 z-10 top-full hidden overflow-hidden" style="display: none;">
                            <ul style="min-width: 100px; text-align: center">
                              <li class="text-sm text-bgray-900 cursor-pointer px-5 py-2 hover:bg-bgray-100 hover:dark:bg-darkblack-600 dark:text-white font-semibold">
                                <a href="{{localeRoute('admin.nomenklature.edit',$nomenklature)}}" class="inline-flex h-8 w-8 translate-y-0 transform items-center justify-center transition duration-300 ease-in-out hover:-translate-y-1">
                                  {{ __('main.edit') }}
                                </a>
                              </li>
                              <li class="text-sm text-bgray-900 cursor-pointer px-5 py-2 hover:bg-bgray-100 hover:dark:bg-darkblack-600 dark:text-white font-semibold">
                                <form action="{{localeRoute('admin.nomenklature.destroy',$nomenklature)}}}" method="POST">
                                  @csrf
                                  @method('PUT')
