...
@section('title',__('main.payments'))
{{-- Content --}}
@section('content')
          ...
          <div class="card-body">
            <table class="w-full">
              ...
              <tr class="border-b border-bgray-300 dark:border-darkblack-400">
                ...
                <td class="text-base font-medium text-bgray-600 dark:text-bgray-50 px-6 py-5">{{__('main.username')}}</td>
                ...
                <td class="sorting text-base font-medium text-bgray-600 dark:text-bgray-50 px-6 py-5" data-sort="payment_system"><strong>{{__('main.payment_system')}}</strong> {!! \App\Helpers\QueryHelper::getDirectionLabel('payment_system') !!}</td>
              ...
              @if(isset($payments))
                @foreach($payments as $payment)
                  <tr class="border-b border-bgray-300 dark:border-darkblack-400 hover:bg-gray-300 clickable-row">
                    ...
                    <td class="px-6 py-5">{{ $payment->user->getFullname() }}  </td>
                    <td class="px-6 py-5">{{ $payment->tarif->getTitle() }}</td>
                    <td class="px-6 py-5">{{ $payment->amount }}</td>
                    <td class="px-6 py-5">{{ $payment->payment_system }}</td>
                    <td class="px-6 py-5">{{ $payment->created_at->format('Y-m-d H:i:s') }}</td>
                    <td class="px-6 py-5">
                      <div class="payment-select relative">
                        <button onclick="dateFilterAction('#cardsOptions{{$payment->id}}')" type="button">
                        ...
                        <div id="cardsOptions{{$payment->id}}" class="rounded-lg shadow-lg min-w-[100px] bg-white dark:bg-darkblack-500 absolute right-10 z-10 top-full hidden overflow-hidden" style="display: none;">
                          <ul style="min-width: 100px; text-align: center">
                            <li class="text-sm text-bgray-900 cursor-pointer px-5 py-2 hover:bg-bgray-100 hover:dark:bg-darkblack-600 dark:text-white font-semibold">
                              <a href="{{localeRoute('admin.payment.edit',$payment)}}" class="inline-flex h-8 w-8 translate-y-0 transform items-center justify-center transition duration-300 ease-in-out hover:-translate-y-1">
                                {{ __('main.edit') }}
                              </a>
                            </li>
                            <li class="text-sm text-bgray-900 cursor-pointer px-5 py-2 hover:bg-bgray-100 hover:dark:bg-darkblack-600 dark:text-white font-semibold">
                              <form action="{{localeRoute('admin.payment.destroy',$payment)}}}" method="POST">
                                @csrf
                                @method('PUT')
